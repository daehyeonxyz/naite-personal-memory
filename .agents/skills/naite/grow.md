...
All data paths below (`roots/articles/`, `roots/conversations/`, etc.) resolve against **NAITE_ROOT** (the root of the naite vault). Sub-skill references resolve against **SKILL_DIR** (`<NAITE_ROOT>/.agents/skills/naite`). See `SKILL.md` for context.
