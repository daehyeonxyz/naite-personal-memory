...
**First action on every invocation**: read `<NAITE_ROOT>/AGENTS.md` in full. For ask, care, grow, fruit, or any tree mutation, also read `<NAITE_ROOT>/CONTEXT.md` before selecting evidence files.
...
- **Secrets policy from `AGENTS.md` is absolute.** If care flags a secret, halt before any git operation and report.
