...
모든 데이터 경로는 NAITE_ROOT 기준으로, 하위 스킬 참조는 SKILL_DIR (`<NAITE_ROOT>/.agents/skills/naite`) 기준으로 푼다. 맥락은 SKILL.md 를 본다. 사용자 대면 카피와 export 품질 기준은 `docs/QUALITY.md` 가 단일 소스다.
