...
- **`WIKI_ROOT`**: the root of your naite repo — the directory containing `AGENTS.md`, `wiki/`, and `raw/`. All data paths (`wiki/`, `raw/`, `AGENTS.md`, `CONTEXT.md`, `index.md`, `log.md`) resolve against this.
- **`SKILL_DIR`**: `<WIKI_ROOT>/.agents/skills/wiki` — this skill and its sub-files. Optionally junction/symlink this into your user-level skills directory (e.g. `~/.agents/skills/wiki`) so `/wiki` resolves from any working directory.

**First action on every invocation**: read `<WIKI_ROOT>/AGENTS.md` in full. It defines the bootloader rules you must respect. Do not skip this read. For query, lint, curate, ingest, course, synapse, or any wiki mutation, also read `<WIKI_ROOT>/CONTEXT.md` before selecting evidence files.
...
| `synapse [topic?]` | 의사결정 thread (DMU) 를 wiki 의 시냅스 layer 로 박는 dialogue scaffold. 14 섹션을 차례 질문으로 끌어내고 trade-off / failure / invariant / cross-link 을 강제. 명시 슬래시 외에도 대화 중 결정 패턴 또는 작업 종료 시그널 감지 시 Codex 가 자동 제안. CONVENTIONS.md § Decision thread shape 참조. | `<SKILL_DIR>/synapse.md` |
...
`<SKILL_DIR>` = `<WIKI_ROOT>/.agents/skills/wiki`. Substitute when reading.
...
2. Read `<WIKI_ROOT>/AGENTS.md` if you haven't this session.
...
- **Secrets policy from `AGENTS.md` is absolute.** If lint flags a secret, halt before any git operation and report.
