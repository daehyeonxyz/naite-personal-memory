...
All data paths below resolve against **WIKI_ROOT** (the root of the naite repo). Sub-skill references resolve against **SKILL_DIR** (`<WIKI_ROOT>\.agents\skills\wiki`). See `SKILL.md` for context.
...
세부 워크플로는 `.agents/skills/wiki/course-backfill.md` 가 담당. 본 op 는 분기만 책임진다.
...
- 과목 시작 시점에 빈 챕터·서브챕터 stub을 미리 생성하지 않는다 (`.agents/skills/wiki/study.md` 규칙과 동일).
