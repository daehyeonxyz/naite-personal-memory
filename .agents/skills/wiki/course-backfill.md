...
- **Subchapter shape**: `.agents/skills/wiki/course.md § Templates § 서브챕터 노트` 그대로. Lecture-natural H-tag, `## Source` trailing provenance (file path only), 본문에 원본 PDF page anchor·이미지 임베드 안 함.
...
3. `## Source` 에 `.txt` path 1개만 (또는 1 subchapter 가 multiple `.txt` 로 매핑되면 모두 나열). 중간 산출물 (`-ko.md` 같은 부분 번역·요약) 은 raw 에 두지 않음 — `raw/` content-immutable, distillation 은 wiki 가 담당 (`AGENTS.md § Layers`).
...
1. `course.md § 0` 의 모든 step (AGENTS.md, index.md, log.md tail).
...
4. 각 subchapter 에 대해 다음 a-h 를 정확히 수행:
   ...
   - **d. Subchapter 페이지 작성** — `§ Codex prompt template` 의 골격 + `.agents/skills/wiki/course.md § Templates § 서브챕터 노트`. style anchor 참조. 0-to-1 신규 작성 시 frontmatter 5 facet 신규 생성, deepening pass 시 기존 frontmatter 보존 + 본문 *추가* (축소·삭제 금지).
...
>
> **Page shape**: `.agents/skills/wiki/course.md § Templates § 서브챕터 노트` 그대로 준수.
...
- 본문에 원본 PDF page anchor 인용 (`.agents/skills/wiki/course.md § Templates § 서브챕터 노트` self-contained 룰).
