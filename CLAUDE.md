...
This file is the **entrypoint**. It carries routing, triggers, and hard safety rules — nothing else. Context loading rules live in `CONTEXT.md`. Detailed operating rules live in `CONVENTIONS.md`. Workflow procedures live in `.claude/skills/wiki/<workflow>.md`. Schema rationale lives in `ARCHITECTURE.md`. Canonical vocabularies and generated agent maps live in `ontology/`.
...
- **존댓말로 답한다.** Claude 와 Codex 는 사용자의 직원 또는 비서이므로, 사용자에게 응답할 때는 항상 존댓말 (`~합니다`, `~입니다`, `~드립니다`) 을 사용한다. 시스템 / 운영 진단을 짧게 적는 경우에도 존댓말을 유지한다.
...
- `.claude/skills/wiki/` — workflow procedures (auto-mirrored to `.agents/skills/wiki/`).

---

## Before any wiki mutation

**Read `CONTEXT.md`, `CONVENTIONS.md`, plus the relevant workflow file under `.claude/skills/wiki/`.** Do not improvise context loading, schema, naming, log format, frontmatter facets, or page shape — invariants live in those files. This is non-negotiable: a wiki edit without those reads is drift.
...
|---|---|---|
| Single-source learning finish ("마무리/끝/wiki 반영해줘", paper/article/lecture done) | `/wiki study` | `.claude/skills/wiki/study.md` |
| Course-unit learning (syllabus, course code, multi-chapter material, "Ch1 끝", "이번 학기 X") | `/wiki course` | `.claude/skills/wiki/course.md` |
| Question against existing wiki | `/wiki query` | `.claude/skills/wiki/query.md` |
| Raw file → wiki page conversion | `/wiki ingest <path>` | `.claude/skills/wiki/ingest.md` |
| Conversation → claim summary in `raw/conversations/` | `/wiki capture` | `.claude/skills/wiki/capture.md` |
| Decision/trade-off thread to install as synapse | `/wiki synapse` | `.claude/skills/wiki/synapse.md` |
| Health check (orphans, stubs, drift, output guard, secrets, binary creep) | `/wiki lint` | `.claude/skills/wiki/lint.md` |
| Qualitative review, repair, large sweep, or recurring-rule learning | `/wiki curate` | `.claude/skills/wiki/curate.md` |
