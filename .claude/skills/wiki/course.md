...
과목 단위 학습 세션. 세션 시작부터 끝까지 상주하며 튜터/리마인드 모드로 진행하고, 서브챕터마다 wiki에 페이지를 ingest한다. `study`가 단발성 학습 이벤트라면 `course`는 구조화된 **과목(course) 단위 작업**.
...
Read `CONTEXT.md` before any course mutation. Course work is source-heavy and contract-heavy, so `subchapter-note` and `backfill` should use the Reader / Writer / Verifier split from `CONTEXT.md` whenever the active tool surface supports it and the user has authorized delegation.
...
- **Verifier** runs content guard, rebuilds `ontology/wiki-manifest.json` and `ontology/wiki-dependencies.json`, checks touched pages, and surfaces inbound semantic dependents.
...
| | `/wiki study` | `/wiki course` |
|---|---|---|
| 단위 | 단발성 학습 이벤트 (논문 1편, 아티클 1편, 강의 1편) | 과목 (10-20 챕터, 100+ 페이지) |
| 세션 | 1회 마무리 시점에 capture + ingest | 세션 전체 상주, 서브챕터마다 개별 ingest |
| 출력 | 보통 1~3개 페이지 | 과목 메타 + 챕터 메타 + 서브챕터 노트 (수십 페이지 누적) |
| 스테이징 | `raw/articles/` | `raw/courses/{slug}/` |

단발성인지 과목 단위인지 불명확하면 사용자에게 물음.

## Hard rules

- **Staging**: 강의자료 pdf/이미지는 `raw/courses/{slug}/` 아래 **정규화된 파일명**으로 staging. flat 구조.
- **Filename convention (wiki pages)** — `CONVENTIONS.md § Naming` (lowercase-kebab-case, 영문) 준수:
...
- **Subject**: course 페이지는 그 과목이 다루는 **콘텐츠 path 1개** (예: `[statistics]` 또는 더 narrow `[engineering-math/ode]`). 메타·챕터·서브챕터 모두 동일 단일 path 또는 narrower. Canonical tree: `ontology/subject-tree.md`. `course`, `course-{slug}` 같은 컬렉션 태그는 subject 에 절대 넣지 않는다 — `CONVENTIONS.md § Ontology` 참조. Course 멤버십은 파일명 prefix `course-{slug}-*` 로 보장. `domains` 는 lint 가 subject 의 top-level 을 cache.
- **Index 분리**: index.md 에는 과목 메타 1줄만 등록 (`## Courses § <institution>` 섹션). 챕터/서브챕터 발견 경로는 `course-{slug}-00-index.md § Chapters` → `course-{slug}-ch{NN}-00-index.md § Subchapters` drill-down. **index.md 에 서브챕터/챕터 절대 나열하지 않는다.**
- **Ingest 단위**: **서브챕터 단위 즉시 페이지 작성**. 챕터 메타는 챕터 완료 시점에 일괄. **단 log.md 에는 서브챕터마다 쓰지 않는다** — frontmatter `created`/`updated` 가 정보 운반. log entry 는 `course-chapter` 마무리 시점에 1줄 (subchapter 수만 명시), `course-start`/`course-finish` 도 1줄씩.
- **`/wiki ingest` 호출 금지**: `ingest.md`는 "raw 파일 하나 → 여러 페이지"용 워크플로. `course`는 "대화 맥락 → 페이지 1개"라 구조 불일치. 단 **결과물 규격**(frontmatter, `index.md`·`log.md` 업데이트 포맷)은 `ingest.md` 와 정합되게 맞춘다.
- **Raw 보존**: `raw/courses/{slug}/*.pdf`는 서브챕터 ingest 시점에 `_archive/`로 옮기지 않는다. **과목 완료(`course-finish`) 시점에 일괄 이동.** (이유: 여러 서브챕터에서 같은 pdf를 페이지 범위로 참조하므로 챕터 진행 중엔 원본이 살아있어야 함.)
- **유일한 archive**: `raw/courses/_archive/` 는 이 프로젝트 전체에서 **유일하게 존재하는 `_archive/` 디렉토리**다. `raw/articles/` 와 `raw/conversations/` 에는 archive layer 가 없다 (파일은 제자리 상주, `conversations/` 의 claim summary 만 ingest 후 삭제). `CONVENTIONS.md § Post-ingest handling` 참조.
- **학술 정보만**: syllabus/about에서 수업 시간, 교수명, 시험 일정, 평가 기준 등 행정 정보는 wiki에 담지 않는다. 내용·범위·선후수 관계만.
- 기타 `CLAUDE.md § Secrets & privacy`, `§ Obsidian co-editing` (operational gotchas), `CONVENTIONS.md § Schema evolution` 전부 그대로 적용.
...
스킬 호출 시 **사용자 의도 + 현재 wiki 상태**를 보고 아래 중 하나로 자동 분기. 분기 직전 **1줄로 사용자에게 확인** ("지금 {op} 맞지?"). 혼동되면 선택지 제시.

| Op | 트리거 | Wiki 변경 |
|---|---|---|
| `start` | 신규 과목 시작. slug가 `wiki/index.md § Courses` 어느 institution 섹션에도 없음 | 과목 메타 생성, index·log 갱신 |
| `resume` | 기존 slug가 이미 존재 | 없음 (진행 상태만 요약) |
| `chapter-start` | 새 챕터 pdf 업로드, 챕터 시작 선언 | 없음 (staging + 맥락 세팅만) |
| `subchapter-note` | "1.3 정리해줘", "이 서브챕터 반영" | 서브챕터 노트 1개 생성 (+선택적 일반 개념 페이지). **log.md 작성 없음.** |
| `chapter-finish` | "이 챕터 끝", 챕터 메타 요청 | 챕터 메타 생성, 과목 메타 갱신, log 1줄 |
| `course-finish` | "과목 끝", "학기 종강" | 과목 메타 상태 변경, raw archive, log 1줄 |
| `stage-source` | start/chapter-start 밖에서 자료만 업로드됨 | 없음 (raw 쪽만 staging) |
| `backfill` | 사용자 학습 완료 과목 mass automation, `/wiki course backfill {slug}` 명시 호출 | chapter loop sweep — `course-backfill.md § Workflow` 로 위임 |

## Workflow

### 0. Pre-flight (every invocation)

1. Read `<WIKI_ROOT>/CLAUDE.md`.
2. Read `<WIKI_ROOT>/CONTEXT.md`.
3. If `<WIKI_ROOT>/ontology/wiki-manifest.json` is missing or stale for the current task, run `python scripts/build-wiki-manifest.py`.
4. Read `<WIKI_ROOT>/ontology/wiki-manifest.json` before searching for existing course, concept, or entity pages.
5. Read `<WIKI_ROOT>/ontology/wiki-dependencies.json` before changing existing course or concept pages when semantic dependents may need review. If missing, run `python scripts/build-wiki-dependencies.py`.
6. Read `<WIKI_ROOT>/wiki/index.md` — 특히 `## Courses` 섹션 (institution 그룹별 과목 메타 목록) 과 `## Knowledge domains § <domain>` 의 hub 페이지들.
7. Read last ~30 lines of `<WIKI_ROOT>/wiki/log.md` — 최근 `course-*` 엔트리로 진행 상태 파악 (course-note 는 더 이상 log 에 없음 — frontmatter `updated` 또는 `course-{slug}-*` 파일 mtime 으로 파악).
8. Op 결정, 사용자에게 1줄 확인.
...
2. Slug 결정:
   ...
   - 없음 → 기관 prefix + NNN 제안. `wiki/index.md § Courses` 와 `raw/courses/` 디렉토리에서 기존 slug 확인 후 다음 번호.
   - slug는 반드시 단일 토큰 (영소문자·숫자, 하이픈 없음).
3. Staging: `raw/courses/{slug}/` 생성. 업로드 자료 복사 (`§ Staging rules` 참조).
4. Takeaways 논의 (3-8 bullet):
   ...
   - 기존 wiki 개념 페이지와의 접점 (`[[probability]]` 있으면 메타 페이지에서 링크)
   - 유사 과목/선후수 관계
   사용자 확인 후 진행.
5. `wiki/course-{slug}-00-index.md` 작성 (`kind=source-record`, `form=index`). 템플릿 `§ Templates § 과목 메타` 사용. frontmatter `subject` 는 단일 path; `domains` 는 빈 배열로 두면 lint 가 cache 채움.
6. `wiki/index.md` 업데이트:
   - `## Courses § <institution>` 섹션에 한 줄: `- [[course-{slug}-00-index]] — {과목명, 짧은 설명}`. institution 섹션이 없으면 신설.
   - **챕터/서브챕터는 절대 index 에 나열하지 않는다.** drill-down 으로 발견.
7. `wiki/log.md`:
   ```
   ## [YYYY-MM-DD] course-start | course-{slug}
   - pages created: [[course-{slug}-00-index]]
   - subject: <path>  (ontology/subject-tree.md 참조)
   - staged: raw/courses/{slug}/<files>
   ```
8. Checkpoint: "과목 셋업 완료. 첫 챕터 자료 주면 `chapter-start` 로 넘어갈게."

### B. `resume` — 기존 과목 재개

1. `wiki/course-{slug}-00-index.md` 읽음.
2. `grep "^## \[" wiki/log.md | grep "course-{slug}"` 로 진행 이력 파악 (course-start, course-chapter, course-finish 만 보임).
...
6. 파일 쓰지 않음. `log.md`에도 쓰지 않음.

### C. `chapter-start` — 새 챕터 진입

1. 챕터 자료 업로드 확인. 기본 staging 이름: `raw/courses/{slug}/ch{NN}-lecture.pdf`. 기존 파일 있으면 덮어쓰기 전 사용자 확인.
2. PDF Read 혹은 텍스트 추출. 추출 품질이 나쁘면(스캔본·OCR 필요 등) **중단**하고 사용자에게 보고. `log.md`에 `aborted` 엔트리 append.
3. 목차·서브챕터 리스트 파악. 사용자에게 "이 챕터 서브챕터는 {리스트} 맞지?" 확인.
4. 파일 쓰지 않음. 이 단계는 맥락 세팅만 (리마인드·설명은 이제부터 진행). `log.md`에도 쓰지 않음 — 챕터 메타는 `chapter-finish` 에서만 작성.
...
2. `wiki/index.md` 재확인 — 관련 기존 hub 컨셉 페이지가 있는지 (`[[laplace-transform]]`, `[[generative-ai]]` 등). `[[...]]` 연결 후보 수집.
3. 사용자에게 **takeaways 3-8 bullet** 제시 (`ingest.md § 4` 원칙 그대로):
   ...
   - 강의 내용에 **trade-off / 결정 / 실패 분석**이 들어가 있으면 서브챕터 노트와 별도로 `/wiki synapse` 페이지로 분리할지 — `CONVENTIONS.md § Decision thread shape` 참조.
   사용자 확인 후 진행.
4. 서브챕터 노트 파일 작성:
   - 경로: `wiki/course-{slug}-ch{NN}-{SS}-{title-slug}.md`
   - Frontmatter: `kind=source-record`, `form=prose`, `domains: []` (lint 가 subject 에서 cache)
   - 본문: `§ Templates § 서브챕터 노트` 참조.
5. **일반 개념 페이지 자율 생성** (`§ Schema autonomy` autonomy A): step 3 에서 식별된 추출 후보 — 사용자가 명시적으로 빼라고 한 것을 제외 — 를 이 시점에 별도 Write. frontmatter 5 facet 은 `ingest.md § 5` 규격. `topics` / `subject` 는 `ontology/` canonical 우선; 미등록 새 topic 이 입자도 가드 통과하면 `ontology/topics.md § canonical_topics` 에 직접 append (autonomy A); 새 narrower 가 자연스러우면 `ontology/subject-tree.md § narrower:` 에 candidate append + chapter-finish log 의 surface 항목으로 기록 (autonomy B). 새 일반 페이지가 hub 후보면 (다른 페이지에서 자주 link 받을 만하면) `index.md § Knowledge domains § <domain>` 의 "주요" 라인에 추가 검토.
6. `wiki/index.md` 업데이트:
   - **서브챕터 노트는 index 에 등록하지 않는다.**
   - 새 일반 개념 페이지가 hub 자격이 있다면 `## Knowledge domains § <domain>` 의 "주요" 리스트에 한 줄 추가 (4-7개 한도 내에서).
   - 기존 hub 페이지의 요약이 본질적으로 바뀌면 한 줄 revise.
7. `wiki/log.md`: **작성 안 함.** frontmatter `created`/`updated` 가 정보 운반.
8. Content guard: 방금 쓴/수정한 페이지의 `## Source` 앞 body 를 `/wiki curate § Content Guard` 기준으로 스캔하고, raw/source voice·불필요한 영어 generic heading·mojibake 를 즉시 고친다.
9. Rebuild generated maps:
   ```powershell
   python scripts/build-wiki-manifest.py
   python scripts/build-wiki-dependencies.py
   ```
10. Inspect `ontology/wiki-dependencies.json` for inbound references to touched slugs. Surface semantic dependent candidates only; do not auto-rewrite them during `subchapter-note`.
11. Checkpoint: "반영 완료. 다음 서브챕터?"

### E. `chapter-finish` — 챕터 마무리

1. 해당 챕터 서브챕터 노트 전부 존재 확인 (glob `wiki/course-{slug}-ch{NN}-*` 에서 `-00-index` 제외).
2. 누락 있으면 사용자 확인 — 의도적 스킵이면 진행, 실수면 해당 서브챕터 먼저 `subchapter-note`.
3. 챕터 메타 파일 작성: `wiki/course-{slug}-ch{NN}-00-index.md` (`kind=source-record`, `form=index`). `§ Templates § 챕터 메타`. `domains` 는 lint cache.
4. `wiki/course-{slug}-00-index.md`의 Chapters 섹션 업데이트 (챕터 상태를 "완료"로, 요약 line refresh).
5. **index.md 갱신 없음** (챕터 메타는 course 메타에서만 발견 — index 직접 등록 안 함).
6. `wiki/log.md`:
   ```
   ## [YYYY-MM-DD] course-chapter | course-{slug} Ch{NN} — {Chapter Title}
   - subchapters: N — {slug list}
   - source: raw/courses/{slug}/ch{NN}-lecture.pdf
   - drafter: {cowork | code | codex}
   ```
7. **Git commit (chapter = atomic unit, push 안 함)**:
   ```
   git add wiki/course-{slug}-ch{NN}-*.md \
           wiki/course-{slug}-00-index.md \
           wiki/log.md \
   ...
   ```
   - **Drafter tag** (필수): `cowork` (Claude desktop app Cowork 탭), `code` (Claude Code CLI 또는 desktop app Code 탭), `codex` (OpenAI Codex CLI). 사후 품질 비교·트래킹용 — `git log --grep "drafter=codex"` 같은 쿼리로 surface.
   ...
   - 첫 챕터 commit 은 untracked `course-{slug}-00-index.md` (course-start 산출물) 도 함께 staging.

### F. `course-finish` — 과목 종료

1. `wiki/course-{slug}-00-index.md` 상태 변경 (frontmatter `updated`, 본문 `상태: 완료 (YYYY-MM-DD)`).
2. `raw/courses/{slug}/` 전체를 `raw/courses/_archive/{slug}/`로 이동. `_archive/` 없으면 생성.
3. `wiki/log.md`:
   ```
   ## [YYYY-MM-DD] course-finish | course-{slug}
   - chapters: N
   - subchapter pages: M
   - archived: raw/courses/_archive/{slug}/
   ```
4. **Git commit + push (과목 atomic 종료 + 원격 동기)**:
   ```
   git add wiki/course-{slug}-00-index.md wiki/log.md raw/courses/_archive/{slug}/
   git add ontology/wiki-manifest.json ontology/wiki-dependencies.json
...
4. `log.md`에 쓰지 않음 — raw 레이어 staging만이므로.

### H. `backfill` — Codex automation 모드

세부 워크플로는 `.claude/skills/wiki/course-backfill.md` 가 담당. 본 op 는 분기만 책임진다.

1. 트리거: 사용자가 `/wiki course backfill {slug}` 또는 `/wiki course start {slug} --mode=backfill` 호출. 자동 추론 안 함 — `§ 0 step 4` 의 op 결정 단계에서 사용자 입력으로 명시.
2. `§ A start` 의 step 1-3 (slug 결정, staging) 그대로 진행.
3. step 4-7 의 takeaways·dialogue 단계 **건너뛰기**. backfill 은 사용자 mental model 이 이미 안정된 콘텐츠 대상이므로 dialogue 가 페이지 가치에 기여하지 않는다. 다만 **`§ Schema autonomy` 의 autonomy A 는 그대로 적용** — 일반 개념 페이지 추출, canonical topic 추가, 명백한 alias 추가 모두 자율 진행 (입자도 가드 통과 시). autonomy B (narrower 제안) 는 candidate append + chapter-finish log 의 surface 항목으로 기록, autonomy C 는 발견 시 log 에 명시 후 사용자 결정 대기. backfill 의 "dialogue 생략" 은 *큐레이션 대화 생략*이지 *연결 생성 생략*이 아니다 — 일반 개념 페이지 추출이 빠지면 chapter silo 가 되어 graph 가 빈약해진다.
4. `course-backfill.md § Workflow` 의 chapter loop 으로 위임.
5. 모든 chapter 완료 후 `course-finish` 는 본 파일의 `§ F` 그대로 사용자 명시 승인 후 수행 (자동 push 포함).

backfill 모드는 `active` 모드 (`§ A`) 와 동시에 사용하지 않는다 — 학습 중 콘텐츠에 backfill 을 쓰면 dialogue takeaway 가 사라져서 페이지 가치가 떨어진다.

## Staging rules (`raw/courses/{slug}/`)

**flat** 구조. 한 챕터에 자료 3개 넘어가면 `/wiki lint`가 flag → nested 재구성 검토.
...
```powershell
Copy-Item "{원본 폴더}\{원본파일명}.pdf" "<WIKI_ROOT>\raw\courses\{slug}\ch{NN}-lecture.pdf"
```
3. 복사 완료 후 staging 경로(`raw/courses/{slug}/ch{NN}-lecture.pdf`)에서 렌더링. wiki 페이지 Source 섹션에는 **staging 경로** 기록 (외부 원본 경로 아님 — 경로 누출 방지).
...
**전체 페이지 수 확인 + 렌더링** (WIKI_ROOT에서 실행):
```powershell
cd "<WIKI_ROOT>"
python -c "
import fitz
pdf = fitz.open(r'raw/courses/{slug}/ch{NN}-lecture.pdf')
...
**Read 후 즉시 삭제**:
```powershell
Remove-Item "<WIKI_ROOT>\raw\assets\ch{NN}_p*.png" -Force
```

**운영 규칙**:
- PNG는 `raw/assets/`에 임시 — 서브챕터 노트 작성 완료 직후 삭제. git에 들어가면 안 됨.
...
- **Staging**: `raw/courses/{slug}/`  (또는 완료 후 `raw/courses/_archive/{slug}/`)
...
- `raw/courses/{slug}/ch{NN}-lecture.pdf`  (과목 완료 후엔 `raw/courses/_archive/{slug}/ch{NN}-lecture.pdf`)
```
...
서브챕터 페이지의 H-tag 매핑은 강의의 자연 sub-division 을 그대로 따른다. wiki 는 1 subchapter = 1 file 기준이므로, 1 chapter = 1 file 형태의 외부 노트의 H2 가 이 wiki 에서는 페이지 자체 (H1) 에 해당한다.
...
```md
## Source
- `raw/courses/_archive/{slug}/sessionNN-notes.pdf`
```

**원본 소스 참조 금지**: 본문 prose 가 원본 PDF 의 특정 페이지·위치·다이어그램을 가리키는 anchor 를 포함하지 않는다. 페이지 자체 + wiki 의 인접 페이지로 self-contained 되어야 한다. 원본은 검토·보강 용도이지 본문 이해 의존물이 아니다.

**raw/필기/source-processing voice 금지**: `## Source` 앞 본문에는 raw 경로, PDF page, page range, staging, render, backfill, "필기에는", "강의 노트에는", "자료에서는", "이 페이지에서는" 같은 표현을 쓰지 않는다. 손필기와 시각 강조는 해당 개념 설명의 관점·직관·주의점으로 흡수한다.
...
**Style reference (read-only)**: voice anchor 는 `tmp/style-reference/` 아래 manifest 가 관리. backfill / curate sub-skill 이 manifest 가 있으면 참조, 없으면 manifest 의 fallback path 로 원본 노트를 read-only 로 참고. 어느 경우에도 wiki 에 ingest 하지 않는다.
...
- 과목 시작 시점에 빈 챕터·서브챕터 stub을 미리 생성하지 않는다 (`.claude/skills/wiki/study.md` 규칙과 동일).
- 서브챕터 노트 없이 챕터 메타를 작성하지 않는다.
- slug 충돌 허용하지 않는다 (`wiki/index.md § Courses` + `ls raw/courses/` 중복 체크 필수).
- 서브챕터 ingest 시점에 원본 pdf를 `_archive/`로 옮기지 않는다 (챕터·과목 완료 시점까지 보존).
- 기존 `/wiki ingest` 나 `/wiki capture`를 호출하지 않는다 (구조 불일치). 결과물 규격만 정합되게 맞춘다.
- **frontmatter `domains` 에 `course`, `course-{slug}` 같은 컬렉션 태그를 절대 넣지 않는다** (post-2026-04-28 schema). 단일 콘텐츠 도메인만.
- **서브챕터/챕터 메타를 `index.md` 에 등록하지 않는다.** drill-down 으로 발견.
- **`subchapter-note` 시점에 `log.md` 를 갱신하지 않는다.** frontmatter `created`/`updated` 가 운반. 챕터 마무리 (`course-chapter`) 1줄로 묶는다.
- wiki를 건드리지 않는 op(`resume`, `chapter-start`, `stage-source`)에는 `log.md` 엔트리를 남기지 않는다.
- 수업 시간·교수명·시험·평가 기준 등 행정 정보를 wiki에 쓰지 않는다.
- `chapter-finish` / `course-finish` 외 op 에서는 git commit 하지 않는다 — § E step 7, § F step 4 참조. `course-finish` 외 op 에서는 git push 도 하지 않는다.
