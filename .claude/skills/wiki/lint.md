...
- PDF 가 git tracked 인지 확인 — `*.pdf` 는 `.gitignore` 로 차단돼야 함 (CLAUDE.md § Binary files). tracked PDF 발견 시 즉시 surface.
...
Read the last ~50 entries of `wiki/log.md`. Group entries by their `op | title` shape and look for **recurring manual procedures** — same sequence of operations executed ≥ 3 times without a dedicated skill file under `.claude/skills/wiki/`.
...
Report: the candidate procedure, how many times it recurred, and a proposed skill file name. Do not write the skill — propose it for the user to greenlight. Follow `CLAUDE.md` § Phase co-evolution.
...
Report: cluster, count, entries. Suggest a preventive change (a pre-step, a skill update, or a CLAUDE.md rule). Do not auto-fix.
...
- **Offer** to append the summary as a dated block to the project's user-profile memory (the `memory/user_profile.md` Claude maintains for this project). Do **not** overwrite existing content — append only, with a `## [YYYY-MM-DD] refresh` header. Never touch `wiki/` or `raw/` during this check.
...
## Skill candidates (N)
- recurring: `ingest` on YouTube transcripts (5x) — propose `.claude/skills/wiki/ingest-transcript.md`
