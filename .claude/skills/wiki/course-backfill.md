# /wiki course backfill

`course.md § H` 의 sub-op 위임 대상. 사용자가 이미 충분히 학습한 과목을 dialogue 없이 일괄 정리하는 mode.

active 학습 (`course.md § A-G`) 과 분리된 운영 모델 — Codex 단독 실행 가능, 대량 PDF 추출, chapter 단위 sweep, deferred commit 패턴. 본 파일은 backfill 만의 invariant 와 chapter loop 를 담당하고, 공통 step (slug, frontmatter, raw 폴더 셋업, course-finish archive 이동, log 형식) 은 `course.md` 의 해당 섹션을 그대로 따른다.
...
- 사용자가 자동화에 명시 승인 — `/wiki course backfill {slug}` 호출.

active 학습과 동시에 사용하지 않는다 — 학습 중 콘텐츠에 backfill 을 쓰면 dialogue takeaway 가 사라져서 페이지 가치가 떨어진다.

## Hard rules

- **PNG render mandatory (when visual layer present)**: **텍스트 추출 단독 시작 금지.** PDF → PyMuPDF PNG render + image-read 가 *mandatory pre-write step* (`course.md § D step b`). 강의 본체 + 사용자 손필기 + 도형·도식 모두 PNG 로 image-read 한 뒤에야 본문 작성. 텍스트 추출은 시각 강조·손글씨·다이어그램·수식 일부를 누락. 0-to-1 backfill 든 deepening pass 든 동일 적용 — 깊이 보강도 PNG 재로드부터. **단 source 가 시각 레이어 없는 순수 텍스트 (variant G) 면 적용 대상이 부재하므로 룰이 자동 무력화 — 그 경우 `.txt` 자체가 authoritative source 다.**
- **Subchapter shape**: `.claude/skills/wiki/course.md § Templates § 서브챕터 노트` 그대로. Lecture-natural H-tag, `## Source` trailing provenance (file path only), 본문에 원본 PDF page anchor·이미지 임베드 안 함.
- **Output quality contract**: `CONVENTIONS.md § Output quality contract` 준수. 본문은 self-contained Korean prose 로 작성하고, `raw/`, PDF page, staging, render, backfill, "필기에는", "자료에서는" 같은 source/process voice 를 `## Source` 앞에 남기지 않는다. 손필기·시각 강조·보조자료 관점은 개념 설명으로 흡수한다.
- **Style reference**: `tmp/style-reference/{course-or-domain}/manifest.md` 가 있으면 manifest 의 `Included files` + `Fallback path` 그대로 사용. 없으면 manifest 의 fallback path 의 노트 1-7장을 read-only 로 직접 참고. 어느 경우에도 wiki 에 ingest 안 함.
- **자동화 권한 분리**: 본 sub-op 는 wiki 페이지 작성·검증, raw staging 까지만 자동 수행. **VCS mutation (`git add` / `git commit` / `git push`) 은 사용자 또는 chapter-finish 의 명시 승인 후에만**. Codex 환경에서 `.git/index.lock` 같은 권한 실패 검출 시 즉시 중단하고 보고.
...
- 그 외 모든 `course.md § Hard rules` 적용 (slug, frontmatter, naming, index 분리, raw 보존, archive).
...
### E. 저학년 깊이
저학년 때 들은 코스의 backfill 은 *내용 위주* 정리 — 강의자료에 있는 만큼만 prose 화. 깊이 부족을 wiki / log / temp log 어디에도 메타 코멘트로 기록 금지. worked example 임의 추가 금지.

### F. 과제 / 풀이 자료
강의 외에 사용자 과제 PDF 가 별도 폴더로 존재. 사용자 본인의 *계산 직관·풀이 흐름* 이 다른 코스 손필기와 동등한 인사이트 source. subchapter 작성 시 과제 PNG 도 image-read 하여 풀이 관점을 prose 에 녹임. 단 과제 *문제* 자체를 wiki 에 옮기지 않음.
...
3. `## Source` 에 `.txt` path 1개만 (또는 1 subchapter 가 multiple `.txt` 로 매핑되면 모두 나열). 중간 산출물 (`-ko.md` 같은 부분 번역·요약) 은 raw 에 두지 않음 — `raw/` content-immutable, distillation 은 wiki 가 담당 (`CLAUDE.md § Layers`).
4. 다른 hard rules (subchapter shape, frontmatter 5 facet, naming, index 분리) 는 동일 적용.
...
1. Obsidian markdown 은 raw staging 에 원문 그대로 복사하되, wiki 본문으로 그대로 옮기지 않는다. markdown 은 read-only reference 이며 lecture PDF 가 기준 source 다.
...
4. Obsidian note 는 source path 로 남길 수 있지만, 본문 voice 는 wiki 의 lecture-natural prose 로 재구성한다.
...
2. Obsidian coverage 가 충분한 chapter 는 note 의 논리 frame 을 엄격히 따른다. 단, 문장은 wiki prose 로 재작성하고 verbatim transfer 를 피한다.
...
2. audio 는 직접 ingest 하지 않는다. 사전 변환 transcript 를 `raw/courses/{slug}/chNN-recording-transcript.md` 로 staging 한 뒤 Variant G 처럼 텍스트 source 로 cross-reference 한다.
...
1. `course.md § 0` 의 모든 step (CLAUDE.md, index.md, log.md tail).
...
2. STARTED 마킹.
3. `course.md § C chapter-start` 의 step 적용 (PDF 추출, 서브챕터 리스트 확인). 단 사용자 대화는 없음 — 미리 결정된 매핑 또는 PDF 자동 파싱 결과 사용.
4. 각 subchapter 에 대해 다음 a-h 를 정확히 수행:
   - **a. PNG render (mandatory)** — 해당 subchapter PDF page range 를 PyMuPDF 1.5x 로 PNG render. `course.md § PDF rendering pipeline` 의 python 명령. batch 5-15 페이지 적정. 텍스트 추출 단독 시작 절대 금지.
   ...
   - **d. Subchapter 페이지 작성** — `§ Codex prompt template` 의 골격 + `.claude/skills/wiki/course.md § Templates § 서브챕터 노트`. style anchor 참조. 0-to-1 신규 작성 시 frontmatter 5 facet 신규 생성, deepening pass 시 기존 frontmatter 보존 + 본문 *추가* (축소·삭제 금지).
   - **e. Content guard** — 작성/수정한 page 의 `## Source` 앞 body 를 `/wiki curate § Content Guard` 기준으로 스캔하고 즉시 수정. 특히 raw path, source-process voice, unnecessary English generic heading, mojibake 는 DONE 전에 남기지 않는다.
   - **f. PNG 즉시 삭제** — 해당 subchapter 작성 끝나면 `Remove-Item` 으로 raw/assets/{slug}_ch{NN}_{SS}_p*.png 즉시 삭제. 누적 금지.
...
5. Chapter 메타 페이지 작성 — `course.md § E chapter-finish` step 3 그대로. 단 commit 은 안 함.
6. DONE 마킹.
...
- source: raw/courses/{slug}/sessionNN-notes.pdf p.{START}-{END}
...
- index.md proposed line: ` - [[course-{slug}-00-index]] — {title}`
- 1-line course summary
```

finalizer 가 위 정보를 `wiki/log.md` 의 coarse migration / course-start entry 로 변환 후 본 run-log 삭제. **페이지별 verdict 절대 wiki/log.md 에 노출 안 함** — log.md 는 coarse summary 만.

### 2. Course-finish

backfill loop 종료 후 사용자가 명시 승인 시 `course.md § F course-finish` 의 step 그대로 — archive 이동, log entry, push 까지.
...
4. ignored 항목이 `raw/courses/_archive/{slug}/` 아래 source 파일이면, main repo 의 같은 경로에서 `git ls-files` 로 추적 여부를 확인한다. 추적되지 않았으면 main 에 복사 후 `git add -f` + commit 으로 raw source 를 먼저 보존한다.
...
> **목표**: wiki 의 course subchapter page 1개 작성.
>
> **Ontology**: `CONVENTIONS.md § Ontology`, `ontology/subject-tree.md`, `ontology/topics.md`.
>
> **Page shape**: `.claude/skills/wiki/course.md § Templates § 서브챕터 노트` 그대로 준수.
...
> - `## Source` 앞 본문에 `raw/`, staging, PDF page, page range, render, backfill, "필기에는", "강의 노트에는", "자료에서는", "이 페이지에서는" 같은 source/process voice 를 쓰지 않는다.
>
...
>
> **출력**: `wiki/course-{slug}-ch{NN}-{SS}-{title-slug}.md`. frontmatter 5 facet, body 는 hard rules 준수.
...
### Codex scratch 격리
codex (또는 다른 agent) 가 자체 scratch (`.codex-work/`, `.codex-cache/` 등) 를 메인 repo 안에 만드는 경우가 있음. 이 디렉토리는 wiki 와 무관 — `.gitignore` 차단 대상. 발견 시 lint surface (`/wiki lint § 7` Binary creep 의 비-wiki dirt 검사).

## What this command never does

- 사용자 dialogue 없이 active learning 콘텐츠 작성 (이건 `course.md § A start` 의 책임).
- 자동 `git add` / `git commit` / `git push` — 항상 사용자 승인 후.
- 강의자료에 없는 worked example 을 만들어서 페이지에 추가.
- 본문에 원본 PDF page anchor 인용 (`.claude/skills/wiki/course.md § Templates § 서브챕터 노트` self-contained 룰).
- 텍스트 추출 단독으로 본문 작성 (PNG render + image-read 가 mandatory pre-write step).
- `tmp/style-reference/` 또는 `tmp/{slug}-notes-extracted.md` / `tmp/{slug}-exam-outlines.md` 내용을 wiki 로 복사·요약·인용 페이지 생성.
