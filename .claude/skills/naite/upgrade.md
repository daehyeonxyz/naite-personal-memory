...
- upgrade 의 유일한 대상은 하네스 경로다.
  - 정확한 파일 집합은 `.naite/scripts/build-harness-lock.py` 가 정의한다 (루트 진입점과 정책 파일 `CLAUDE.md`·`AGENTS.md`·`SOUL.md`·`README.md`·`LICENSE`·`.gitignore`, 두 스킬 표면, `.claude-plugin/**`, `docs/**`, `.naite/scripts/**`, `.naite/templates/**`, `.naite/hooks/**`)과 `.naite/harness-lock.json` 자신이다.
  - 이 집합의 단일 소스는 그 스크립트다.
- 하네스 디렉터리 안의 사용자 생성 파일(예: 사용자가 `.claude/skills/` 아래에 추가한 커스텀 스킬)은 lock 에도 새 릴리스에도 없다. 건드리지 않고 둔다.
