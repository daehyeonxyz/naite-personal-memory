...
- 페이지가 없으면 완전한 ontology frontmatter 로 `tree/<slug>.md` 를 만든다.
  ...
  - `kind` 선택 기준:
    ...
    - `entity`: 구체적인 도구·사람·조직·제품이다 (Claude Code, Karpathy, OpenAI 등).
...
subject 경로 드리프트와 가드 실패 topic 의 누적은 care --check 의 주된 surface 대상이다. 자율 A 추가물(정본 topic, 별칭, 일반 개념 페이지)의 사후 품질은 care --check 의 garbage collector(`.claude/skills/naite/care-check.md` 14절)가 검증한다.
