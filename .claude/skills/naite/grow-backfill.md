...
- subchapter 의 형태는 `.claude/skills/naite/grow-branch.md` Templates 절의 서브챕터 노트를 그대로 따른다.
...
3. `## Source` 에는 `.txt` 경로 하나만 적는다 (한 subchapter 가 여러 `.txt` 로 매핑되면 전부 나열한다). 중간 산출물(`-ko.md` 같은 부분 번역·요약)은 roots 에 두지 않는다. `roots/` 는 content-immutable 이고 증류는 tree 가 담당한다 (`CLAUDE.md` 계층 구조 절).
...
1. `grow-branch.md` 0절의 모든 단계를 수행한다 (CLAUDE.md 와 trunk.md 와 rings.md 꼬리 읽기).
...
4. 각 subchapter 에 대해 다음 a~h 를 정확히 수행한다.
   ...
   - d. subchapter 페이지 작성: Codex prompt template 절의 골격과 `.claude/skills/naite/grow-branch.md` Templates 절의 서브챕터 노트를 따른다. 문체 anchor 를 참조한다. 0에서 1로 새로 쓸 때는 frontmatter 5 facet 을 새로 만들고, 깊이 보강 pass 에서는 기존 frontmatter 를 보존하고 본문을 추가만 한다 (축소와 삭제는 금지된다).
...
>
> 페이지 형태: `.claude/skills/naite/grow-branch.md` Templates 절의 서브챕터 노트를 그대로 지킨다.
...
- 본문에 원본 PDF 의 페이지 anchor 를 인용하지 않는다 (`.claude/skills/naite/grow-branch.md` Templates 절 서브챕터 노트의 자립 규칙).
