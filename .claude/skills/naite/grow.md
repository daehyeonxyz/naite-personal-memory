...
아래 모든 데이터 경로(`roots/articles/`, `roots/conversations/` 등)는 naite vault 의 루트인 NAITE_ROOT 기준으로 해석한다. 하위 스킬 참조는 SKILL_DIR(`<NAITE_ROOT>/.claude/skills/naite`) 기준으로 해석한다. 맥락은 `SKILL.md` 가 담당한다.
