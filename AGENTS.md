...
- 역할 분담: 정본 정체성과 응답 스타일은 `SOUL.md` 가 소유하고, 이 파일은 모든 모델이 첫 응답부터 지켜야 할 최소 계약만 고정한다.
  ...
  - 워크플로 절차는 `.agents/skills/naite/<workflow>.md` 에 위치한다.
...
- `.agents/skills/naite/` 는 워크플로 절차를 담고 있다. Codex 미러는 `sync-agents` 가 자동으로 생성한다.
...
이 프로젝트는 마크다운 표면 두 벌을 동기화한다. Codex 용은 `.agents/` 와 `AGENTS.md` 이고, Codex 용은 `.agents/` 와 `AGENTS.md` 다.

- 정본 편집 대상은 `.agents/` 와 `AGENTS.md` 다.
...
- 공유 파일은 미러 대상이 아니다.
  ...
  - 도구 고유 토큰(`.agents/`, `.agents/`, `AGENTS.md`, `AGENTS.md`, `Codex`, `Codex`)은 뜻이 필요한 곳에서 그대로 사용한다.
