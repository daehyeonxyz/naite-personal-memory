...
naite 에이전트는 세 가지 표면으로 나를 이해합니다. 모두 평범한 Markdown 파일이라 직접 다듬을 수 있습니다.

- `SOUL.md` 는 에이전트의 정체성과 말투입니다. 기본값이 들어 있고, 톤을 바꾸고 싶으면 이 파일을 고칩니다.
- `USER.md` 는 나의 응답 선호입니다 (어떤 톤과 길이로 답할지 등). 비워 두고 시작해도 되고, `/naite start` 가 동의를 받아 채워 줍니다.
- `MEMORY.md` 는 진행 중인 작업과 운영 메모를 모으는 곳입니다.
